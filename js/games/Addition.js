...
export class Addition extends BaseGame {
  #numpad = null;
  ...
  mount(stage, q, ctx) {
    ...
    stage.innerHTML = `
      <h2 style="font-size:22px; margin-bottom:16px; color:var(--ink-soft);">What is the sum?</h2>
      ...
      <div class="eq-card" style="display:inline-flex; flex-direction:column; align-items:flex-end; padding:24px 36px 20px; box-shadow:var(--shadow-card); min-width:160px;">
        ...
        <div style="display:flex; align-items:center; gap:10px; margin-bottom:12px;">
          <span style="font-size:28px; font-weight:900; color:var(--teal); margin-right:4px;">+</span>
          <span style="font-size:32px; font-weight:900; color:var(--teal);">${q.b}</span>
        </div>
        ...
        <div style="display:flex; align-items:center; gap:10px; align-self:center;">
          ...
          <div class="numpad-display" id="npDisplayHost" style="border:3px dashed var(--teal); width:96px;"><span class="placeholder-q">?</span></div>
        </div>
    ...
    this.#numpad = new Numpad(stage.querySelector('#numpadHost'), {
      maxDigits: 3,
      onChange: (val) => {
        dispHost.textContent = val === null ? '' : val;
        if (val === null) dispHost.innerHTML = '<span class="placeholder-q">?</span>';
        checkBtn.disabled = val === null;
      },
    });
    // The Numpad renders its own display too — hide it since this game
    // shows the running total inline as part of the equation instead.
